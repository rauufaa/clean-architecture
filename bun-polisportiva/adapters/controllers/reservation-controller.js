...
import findRatingById from "../../application/use-cases/reservation/findrating-by-id";
import add from "../../application/use-cases/reservation/add";
import findAll from "../../application/use-cases/reservation/findall";
import updateStateById from "../../application/use-cases/reservation/updatestate-by-id";
import findById from "../../application/use-cases/reservation/find-by-id";
...
export default function reservationController(
    ...
    const getReservation = async ({ request: {
        ...
        let id = params.id
        try {
            const {reservation, owner, sportsField} = await findById(id, reservationDbRepository, sportsFieldDbRepository, userDbRepository, addressDbRepository);
            return {
                statusCode: 200,
                status: "SUCCESS",
            ...
            return {
                ...
                data: {
                    ...
                    sportsField: {
                        ...
                        name: sportsField.detail.name,
                        sport: sportsField.detail.sport,
                        isIndoor: sportsField.detail.is_indoor,
                        soccerFieldType: sportsField.detail.soccer_field_type,
                        tennisFieldType: sportsField.detail.tennis_field_type,
                        owner: {
                            id: sportsField.owner.user.id,
                            username: sportsField.owner.user.username,
                            address: {
    ...
    const postReservationRating = async ({ request: {
        ...
        let ratingVal = body.rating
        let description = body.description

        try {
            const { reservation, rating } = await addRating(id, { ratingVal, description }, reservationDbRepository, reservationsRatingDbRepository);
            return {
                statusCode: 200,
                status: "SUCCESS",
                message: "Success add reservation data",
