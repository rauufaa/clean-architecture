...
export default async function add({
    ...
    try {
        ...
        if (userEmailFound) throw new ResponseError(400, "USER_EMAIL_INVALID", "Bad request", "Email already exist")
        const userUsernameFound = await userDbRepository.findByUsername(username)
        if (userUsernameFound) throw new ResponseError(400, "USER_USERNAME_INVALID", "Bad request", "Username already exist")
